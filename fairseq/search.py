...
class BeamSearch(Search):
    ...
    def step(self, step: int, lprobs, scores: Optional[Tensor]):
        ...
        top_prediction = torch.topk(             # 三维压二维
            lprobs.view(bsz, -1),
            k=min(
        ...
        beams_buf = indices_buf // vocab_size    # 除数
        indices_buf.fmod_(vocab_size)            # 余数
...
class SamplingPlusBeamSearch(Search):
    # 前几步sampling，然后再beam search
    sampling_topk: int
    sampling_topp: float

    def __init__(self, tgt_dict, sampling_step=1, sampling_topk=-1, sampling_topp=-1.0):
        super().__init__(tgt_dict)
        self.sampling_topk = sampling_topk
        self.sampling_topp = sampling_topp
        self.sampling_step = sampling_step    # 前多少个step采用sampling解码
