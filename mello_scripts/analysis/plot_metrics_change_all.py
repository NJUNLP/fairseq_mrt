# 把所有数据集和指标的分析一起画图，打包所有图片
import csv
import os
import numpy as np
import matplotlib.pyplot as plt

path_prefix = 'mrt_analysis_results'
if not os.path.exists(path_prefix + '/all_plot'):
    os.makedirs(path_prefix + '/all_plot')
paths = os.listdir(path_prefix)

metric_name = {'bleu': 'BLEU', 'bertscore': 'BERTScore', 'bartscore': 'BARTScore', 'bleurt': 'BLEURT', \
    'comet': 'COMET', 'unite_ref': 'UniTE_ref', 'unite_src_ref': 'UniTE_src_ref'}

for path in paths:   # 每种 指标*语言对
    if '2' not in path: continue
    lang, metric = path.split('_', 1)
    ...
    plt.savefig(path_prefix + '/all_plot/mrt_' + lang + '_' + metric + '_plot_metrics.jpg')

# python3 mello_scripts/analysis/plot_metrics_change_all.py